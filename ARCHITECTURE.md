...
rust crate `imbhgo` (crate-type = ["staticlib"])   → rust/target/release/libimbhgo.a
   ...
   ├── depends on  imbh   (crates.io 0.6.0; features: cdata, proto, search, serde)
...
- **The arrival (observed-time) axis on log queries. Done (imbh 0.6.0).** `LogQuery.ObservedAfter` (a strict `observed_time > t` bound) and `LogQuery.OrderBy` (`LogOrderTime` \| `LogOrderObservedTime`) ride the one shared `LogQueryWire`, so all four surfaces built on it — `QueryLogs`, `CountLogs`, `QueryLogPage`, `LogVolume`/`LogVolumeBy` — inherit both, and `LogEntry.ObservedTime` decodes the arrival clock from a projection column that was always present. This is what makes a **correct tailer** expressible: event time is not monotone in arrival, so a follow loop advancing a `Start` bound by the newest event time it has seen silently drops a record emitted earlier but ingested later, while arrival order cannot be overtaken that way. NULL `observed_time` (an OTLP producer need not send one) sorts last in both directions and never matches `ObservedAfter`, so an unplaceable record is left out of the loop rather than replayed on every poll. The one glue judgement: `build_log_query` became **fallible** so an unrecognized `order_by` is an error on every one of the four paths rather than a silent fall back to event time — a typo'd axis would otherwise reintroduce exactly the dropped-record bug the axis exists to fix. Verified under `-race`: ordering (including NULL placement in both directions), the strict bound across the four surfaces, a three-poll tailer that delivers a late arrival exactly once (asserting alongside it that the event-time watermark misses it), and the rejection.
