...
```sh
go get github.com/moriyoshi/imbh-go@v0.6.0
...
- `-print-env` emits POSIX shell syntax on **every** platform, including Windows — the shell that
  ...
  ```bat
  for /f "delims=" %i in ('go run github.com/moriyoshi/imbh-go/cmd/imbhgo-fetch@v0.6.0 -print-env -shell cmd') do @%i
  ```

  ```powershell
  go run github.com/moriyoshi/imbh-go/cmd/imbhgo-fetch@v0.6.0 -print-env -shell powershell | Invoke-Expression
  ```
