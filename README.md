...
```sh
go get github.com/moriyoshi/imbh-go@v0.5.1
...
- `-print-env` emits POSIX shell syntax on **every** platform, including Windows — the shell that
  ...
  ```bat
  for /f "delims=" %i in ('go run github.com/moriyoshi/imbh-go/cmd/imbhgo-fetch@v0.5.1 -print-env -shell cmd') do @%i
  ```

  ```powershell
  go run github.com/moriyoshi/imbh-go/cmd/imbhgo-fetch@v0.5.1 -print-env -shell powershell | Invoke-Expression
  ```
...
- **Packaging.** Consumed as external deps — imbh from crates.io (`0.1.1`) and sable git-pinned to a
  `main` commit — so no local checkouts are required to build. CI runs the standard gate on every push
  and pull request; cold build is minutes; the static archive is large.
...
- **Portability.** `linux/{amd64,arm64}` and `windows/amd64` are gated in CI: full build + `go test
  -race`, with the Windows leg building for `x86_64-pc-windows-gnu` and linking through the cgo
  directives natively. Durable (on-disk) DBs need imbh `0.1.1` or newer on Windows — `0.1.0` failed
  every on-disk open there (imbh#3). The Apple cells are compile-checked in CI (`cargo check` on a macOS
  runner) and built on release tags, but the Go suite has never run there.
